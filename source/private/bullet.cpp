				...
				for (int j = 0; j < ENEMY_MAX; j++)
				{
					...
					if (enemy[j].use == TRUE)
					{
						BOOL ans = CollisionBB(g_Bullet[i].pos, g_Bullet[i].w, g_Bullet[i].h,
							enemy[j].pos, enemy[j].w, enemy[j].h) || (g_Bullet[i].pos, g_Bullet[i].w, g_Bullet[i].h,
						...
						if (ans == TRUE)
						{
							...
							EBULLET* g_EBullet = GetEBullet();

							if (enemy[i].use == FALSE, enemy[j].use == FALSE) {
					...
					if (boss[j].use == TRUE)
					{
						...
						if (ans == TRUE)
						{
							...
							BossBullet* g_BossBullet = GetBossBullet();
							...
							// エフェクト発生
							SetEffect(g_Bullet[i].pos.x, g_Bullet[i].pos.y, 30);
							PlaySound(SOUND_LABEL_SE_explosion);
						}
					}
				}
