...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		//* y-axis (forward)
		if ( dijs.lY < 0 || (dijs.rgdwPOV[0] > 27000 && dijs.rgdwPOV[0]!= -1) || (dijs.rgdwPOV[0] >=0 && dijs.rgdwPOV[0] < 9000))					padState[i] |= BUTTON_UP;
		//* y-axis (backward)
		if ( dijs.lY > 0 || (dijs.rgdwPOV[0] < 27000 && dijs.rgdwPOV[0] > 9000))					padState[i] |= BUTTON_DOWN;
		//* x-axis (left)
